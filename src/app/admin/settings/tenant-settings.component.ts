...
import { TenantSettingsServiceProxy, DefaultTimezoneScope, TenantSettingsEditDto, SendTestEmailInput, TenantDashboardServiceProxy,CreateDiscountInput } from '@shared/service-proxies/service-proxies';
...
export class TenantSettingsComponent extends AppComponentBase implements OnInit {
    ...
    saveAll(): void {
        if(this.discountInput.id==null){
            this.discountInput.id = 0;
        }
        this._tenantDashboardService.createOrUpdateDiscount(this.discountInput).subscribe(result=>{
            this.getDiscounts();
            //this.notify.success("Discount Saved Successfully");
        });
    ...
    //     console.log(type);
    //     if(type=='discount'){
    //         this.discountInput.discountable = this.discountInput.discountable > 100?100:this.discountInput.discountable;
    //     }else{
    //         this.discountInput.unDiscountable = this.discountInput.unDiscountable > 100?100:this.discountInput.unDiscountable;
    //     }
