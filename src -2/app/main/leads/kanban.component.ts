...
import { TokenAuthServiceProxy, InquiryServiceProxy, EnquiryUpdateInputDto,EnquiryUpdateServiceProxy,EnquiryContactServiceProxy,InquiryListDto,EnquiryJunkUpdateInputDto, QuotationStatusUpdateInput, EnquiryStatusUpdateInput, EnquiryContactInputDto, Select2ServiceProxy, QuotationServiceProxy, CreateQuotationInput, Select2InquiryDto, Userprofiledto } from "shared/service-proxies/service-proxies";
			...
			if(result.select2data !=null){
			  if(result.select2data.length == 1)
			  {
				...
				if(parseInt(ei.getAttribute("data-itemQuotationId")) >0)
				{
					...
					{
						this._quotationService.quotationRevision(this.QStatusUpdateInput.quotationId).subscribe(result=>{
							if(result){
								this.getTickets('');			
